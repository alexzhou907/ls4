...
def get_data_min_max(records, device='cuda'):
...
def variable_time_collate_fn(batch, args, device=torch.device("cpu"), data_type="train",
    ...
    # # pad to 16x
    # get_pad = lambda x, d: (x.shape[d]//16 +1) *16 - x.shape[d]
    # data_dict["observed_data"] = torch.nn.functional.pad(data_dict["observed_data"], (0,0,0,get_pad(data_dict["observed_data"], 1)))
    # data_dict["observed_tp"] = torch.nn.functional.pad(data_dict["observed_tp"], (0, get_pad(data_dict["observed_tp"], 0)))
    # data_dict["data_to_predict"] = torch.nn.functional.pad(data_dict["data_to_predict"], (0,0,0,get_pad(data_dict["data_to_predict"], 1)))
    # data_dict["tp_to_predict"] = torch.nn.functional.pad(data_dict["tp_to_predict"], (0, get_pad(data_dict["tp_to_predict"], 0)))
    # data_dict["observed_mask"] = torch.nn.functional.pad(data_dict["observed_mask"], (0,0,0,get_pad(data_dict["observed_mask"], 1)))
    # data_dict["mask_predicted_data"] = torch.nn.functional.pad(data_dict["mask_predicted_data"], (0, 0, 0, get_pad(data_dict["mask_predicted_data"], 1)))
    #
    # assert torch.all(torch.isclose(data_dict["observed_data"], data_dict["data_to_predict"]))
    return data_dict
...
if __name__ == '__main__':
    ...
    dataset = PhysioNet('/viscam/u/alexzhou907/01DATA/physionet', train=False, download=True)
